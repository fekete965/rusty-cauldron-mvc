...
VALUES (1, 'flour', 2, 'cup'),
       (1, 'sugar', 2, 'tbs'),
       (1, 'baking powder', 1.5, 'tsp'),
       (1, 'salt', 0.5, 'tsp'),
       (1, 'cinnamon', 1, 'tsp'),
       (1, 'milk', 1, 'cup'),
       (1, 'butter, melted', 4, 'tbs'),
       (1, 'egg', 1, 'large'),
       (1, 'vanilla extract', 1, 'tsp'),
       (1, 'butter', 4, 'tbs'),
       (1, 'heavy cream', 0.25, 'cup'),
       (1, 'brown sugar', 0.25, 'cup'),
...
INSERT INTO ingredients (recipe_id, name, amount, measurement) 
VALUES ((SELECT id FROM recipes WHERE title = 'Butterbeer' AND user_id = 1), 'Pie dough', 1, 'package'),
       ((SELECT id FROM recipes WHERE title = 'Butterbeer' AND user_id = 1), 'Pumpkin puree', 2, 'cups'),
       ((SELECT id FROM recipes WHERE title = 'Butterbeer' AND user_id = 1), 'Sugar', 1, 'cup'),
       ((SELECT id FROM recipes WHERE title = 'Butterbeer' AND user_id = 1), 'Cinnamon', 1, 'tsp'),
       ((SELECT id FROM recipes WHERE title = 'Butterbeer' AND user_id = 1), 'Nutmeg', 0.5, 'tsp'),
       ((SELECT id FROM recipes WHERE title = 'Butterbeer' AND user_id = 1), 'Ginger', 0.5, 'tsp');
